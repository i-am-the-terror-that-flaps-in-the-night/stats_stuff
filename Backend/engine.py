...
WHERE THIS SITS AMONG THE FOUR ANALYSIS MODULES
    This file is the general-purpose one: it knows nothing about livers, or
    NHANES, or the question this project set out to answer. Hand it any
    spreadsheet and it will describe it. Three siblings build the actual study
    on top of it, and the dependency runs strictly one way -- each imports from
    ...
    That order is the whole architecture. A statistic that would be true of any
    column belongs here; anything that assumes ALT is the outcome and sugar is
    the exposure belongs in study.py, because those roles come from a protocol
    rather than from a spreadsheet. Keeping the general engine unable to see the
    specific question is what lets it refuse to guess causal roles.

    One command drives all four -- see cli.py, the fifth file, which is the
    terminal front door and the only one that imports all of them:

        python Backend/cli.py build-cohort   # cohort.py
        python Backend/cli.py study          # study.py
        python Backend/cli.py train-model    # predictor.py
        python Backend/cli.py --column BMI   # this file's tiers, on any CSV
