...
# the whole point of keeping it separate (see _clinical_threshold_analysis).
#
...
# nothing sane sits between the two bands. _check_units() uses that to verify the
# assumed unit before any threshold is applied, and to name the likely actual
# unit when it doesn't fit. It is a sanity check, not a proof -- it can't catch a
# unit swap that preserves magnitude -- but it turns the silent failure into a
# loud one.
#
...
class DataAnalyzer:
    ...
    def _numeric_column_names(self):
        ...
        return [
            ...
            if _coerce_numeric(self.df[col]).notna().mean() >= NUMERIC_THRESHOLD
        ]
    ...
    def _distribution_metrics(self, series):
        ...
        return {
            ...
            "outliers": self._outlier_counts(series, q1, q3, iqr, skewness),
            "log_transform": self._log_transform(series, skewness),
        }

    def _outlier_counts(self, series, q1, q3, iqr, skewness):
        """Count unusually extreme values under two different rules.

        There is no such thing as THE outlier count -- it depends entirely on the
        rule you pick, so we report both common ones and say which suits the data.

        z-score (more than 3 SDs from the mean) assumes a roughly symmetric,
        bell-shaped distribution. On a right-skewed variable -- which describes
        most biomedical measurements, triglycerides and insulin especially -- the
        long tail drags the mean up and inflates the SD, so the rule flags far too
        few high values and sometimes no low ones at all.
        ...
        error or a genuinely extreme person, and only looking at the record tells
        you which. Nothing here is dropped -- these are counts, not a filter.
        """
        counts: dict[str, Any] = {}
        ...
        # An IQR of zero means the middle half of the data is a single repeated
        # value, so the fences collapse onto it and the rule would flag every
        # other value in the column. That is a degenerate answer, not a count of
        # zero outliers, so we say the rule doesn't apply instead of reporting a
        # number that would be read as "none found".
    ...
    def _uncertainty_metrics(self, series):
        ...
        The interval is a statement about the PROCEDURE: if you repeated the whole
        study many times, intervals built this way would contain the true
        population mean 95% of the time. It is not "a 95% chance the true mean is
        inside this particular interval" -- that interval either contains it or it
        doesn't. It also only covers random sampling error; it says nothing about
        selection bias, measurement error, or a survey design (NHANES has one)
        that the calculation ignores.
        """
        n = int(series.count())
        if n < 2:
        ...
        return {
            ...
            "confidence_level": 0.95,
            "covers": "Random sampling error only, assuming a simple random sample.",
        }

    def _group_tests(self, value_column, group_column):
        """Do the groups actually differ? A primary test and a coarser companion.

        ANOVA is the one to read: it uses the values as they are and asks whether
        the group AVERAGES differ. The median-split chi-square asks a deliberately
        cruder question -- is landing in the top half associated with the group --
        and is included for teaching contrast, not because it is a better test.
        See _median_split_chi_square for why it is the weaker of the two.
        """
        ...
        return {
            ...
            "median_split_chi_square": self._median_split_chi_square(
    ...
    def _anova(self, groups):
        ...
        variation in the values that sits BETWEEN groups rather than within them.
        That is the "how much does it matter" number. With NHANES-sized samples a
        p-value near zero routinely comes with an eta-squared of 0.01, meaning the
        groups are reliably different and the difference explains 1% of what is
        going on -- both true at once.
        """
        ...
        result.update(_significance_report(p_value, self._eta_squared(groups)))
        return result
    ...
    def _eta_squared(self, groups):
        ...
        what counts as a meaningful gap in blood pressure is a clinical question,
        ...
        return {
            ...
            "magnitude": _magnitude(eta_squared, 0.01, 0.06, 0.14),
    ...
    def _median_split_chi_square(self, data, value_column, group_column):
        ...
        row "high" or "low" depending on whether it beats the overall median, then
        ask whether that split looks the same across groups.
        ...
        result = self._describe_test_statistic(chi2, df=degrees_of_freedom)
        ...
        result["effect_size"] = self._cramers_v(chi2, table)
        return result

    def _cramers_v(self, chi2, table):
        """Cramer's V: chi-square rescaled to 0-1 so sample size stops dominating.

        A chi-square statistic grows with n, so on 5,000 rows a trivial
        association produces an enormous one. V divides that back out and answers
        "how strongly are these two labels related", independent of how many rows
        you collected. Rough reading: 0.1 weak, 0.3 moderate, 0.5 strong.
        """
    ...
    def advanced_analysis(
        ...
        _covariate_adjustment for why guessing is the one thing it must not do.
        ...
        result: dict[str, Any] = {
            ...
            "adjustment": self._adjustment_block(
                column, exposure, confounders, mediator
            ),
        }
    ...
    def _adjustment_block(self, column, exposure, confounders, mediator):
        """Run the adjusted-association analysis, or explain why we didn't.

        Previously this tier picked the exposure and confounders off the front of
        the column list and adjusted away. That was the most dangerous thing in
        the engine: the arithmetic was right and the meaning was invented. Now a
        missing role produces this explanation instead of a fabricated causal
        story.
        """
        if exposure is None:
            return {
                ...
                "candidates": self._other_numeric_columns(column),
            }

        return self._covariate_adjustment(
            outcome=column,
            exposure=exposure,
    ...
    def _correlation(self, column1, column2):
        ...
        relationship -- common in biology, where a measurement can be dangerous at
        both extremes -- can produce an r near zero. A near-zero r means "no
    ...
    def _covariate_adjustment(self, outcome, exposure, confounders=None, mediator=None):
        ...
        result.update(
            _significance_report(
                line.pvalue,
                {
                    ...
                    "means": "How tightly the group averages follow a straight line.",
                },
            )
        )
        return result
    ...
    def expert_analysis(
        ...
        applying anything -- see _check_units.
        """
        series = self._numbers(column)
        if series.empty:
            return {"error": "No numeric values in that column."}
        ...
        result["clinical_threshold"] = self._clinical_threshold_analysis(
            column, series, clinical_cutoff, cutoff_direction, units
        )
        result["dataset_median_split"] = self._dataset_median_split(column, series)
        result["ratios"] = self._descriptive_ratios()
    ...
    def _check_units(self, series, reference, declared_unit=None):
    ...
    def _clinical_threshold_analysis(
        self, column, series, cutoff=None, direction=None, unit=None
    ):
        ...
        _check_units. A caller-supplied cutoff is applied as given -- you brought
        the number, so you own the units, and the output says so.
        """
        if cutoff is not None:
            cutoff = float(cutoff)
            return self._count_against_cutoff(
                column,
                series,
                cutoff=cutoff,
        ...
        unit_check = self._check_units(series, reference, unit)
    ...
    def _dataset_median_split(self, column, series):
    ...
    def _descriptive_ratios(self):
        ...
        triglycerides = _coerce_numeric(self.df[by_lowercase_name[triglyceride_name]])
    ...
    def _trend_in_proportions(
        self, column, group_column, p_values=None, method="bonferroni"
    ):
    ...
    def _correct_p_values(self, p_values, method="bonferroni"):
        ...
        will look "significant" by luck. This raises the bar to compensate for how
        many tests were run.
        ...
        method="fdr_bh" controls the false discovery RATE instead and is the usual
        choice when screening many variables.
        """
        ...
        if table.shape[0] >= 2 and table.shape[1] >= 2:
            ...
            if smallest_expected < 5:
                chi_square["assumption_warning"] = (
                    f"The smallest expected cell count is {_num(smallest_expected)}, "
                    "below the usual minimum of 5, so this p-value is approximate. "
                    "Fisher's exact test is the safer choice on a sparse table."
                )
            chi_square.update(
                _significance_report(p_value, self._cramers_v(chi2, table))
            )
